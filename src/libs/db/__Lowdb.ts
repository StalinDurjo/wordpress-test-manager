...
export class Lowdb {
  ...
  constructor(databaseFilePath: string) {
    ...
    // let adapter: any;
    // import('lowdb').then(({ LowSync }) => {
    //   // console.log(LowSync);
    //   import('lowdb/node').then(({ JSONFileSync }) => {
    //     // console.log(JSONFileSync);

    //     adapter = new JSONFileSync<DataSchema>(databaseFilePath);
    //     this.db = new LowSync<DataSchema>(adapter, this.defaultData);
    //   });
    // });

    this.init(databaseFilePath);
  }

  init(databaseFilePath: any) {
    let adapter: any;
    import('lowdb').then(({ LowSync }) => {
      // console.log(LowSync);
      import('lowdb/node').then(({ JSONFileSync }) => {
